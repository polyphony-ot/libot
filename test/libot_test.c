...
MU_TEST(test_serialize_empty_op) {
    const char* const EXPECTED =
        "{\"clientId\":0,\"parent\":\"00\",\"hash\":\"00\",\"components\":[]}";
    ot_op* op = ot_new_op();
    ...
    char msg[1024];
    sprintf(msg, "Serializing empty op did not create expected string. "
                 "Expected = \"%s\". Actual = \"%s\".",
            EXPECTED, actual);
    free(actual);
    ot_free_op(op);
...
MU_TEST(test_serialize_single_insert) {
    const char* const EXPECTED = "{\"clientId\":0,\"parent\":\"00\",\"hash\":"
                                 "\"00\",\"components\":[{\"type\":\"insert\","
                                 "\"text\":\"any string\"}]}";
    ot_op* op = ot_new_op();
    ...
    char msg[1024];
    sprintf(msg, "Serializing a single insert did not create expected string. "
                 "Expected = \"%s\". Actual = \"%s\".",
            EXPECTED, actual);
    free(actual);
    ot_free_op(op);
...
MU_TEST(test_serialize_two_inserts) {
    const char* const EXPECTED = "{\"clientId\":0,\"parent\":\"00\",\"hash\":"
                                 "\"00\",\"components\":[{\"type\":\"insert\","
                                 "\"text\":\"any string any other string\"}]}";
    ot_op* op = ot_new_op();
    ot_insert(op, "any string");
    ...
    mu_assert(cmp == 0,
              "Serializing two inserts did not create expected string.");
}

MU_TEST(test_serialize_single_skip) {
    const char* const EXPECTED = "{\"clientId\":0,\"parent\":\"00\",\"hash\":"
                                 "\"00\",\"components\":[{\"type\":\"skip\","
                                 "\"count\":1}]}";
    ot_op* op = ot_new_op();
    ot_skip(op, 1);
    ...
    mu_assert(cmp == 0,
              "Serializing a single skip did not create expected string.");
}

MU_TEST(test_serialize_single_delete) {
    const char* const EXPECTED = "{\"clientId\":0,\"parent\":\"00\",\"hash\":"
                                 "\"00\",\"components\":[{\"type\":\"delete\","
                                 "\"count\":1}]}";
    ot_op* op = ot_new_op();
    ot_delete(op, 1);
    ...
    mu_assert(cmp == 0,
              "Serializing a single delete did not create expected string.");
}

MU_TEST(test_serialize_single_open_element) {
    const char* const EXPECTED =
        "{\"clientId\":0,\"parent\":\"00\",\"hash\":\"00\",\"components\":[{"
        "\"type\":\"openElement\",\"element\":\"any string\"}]}";
    ot_op* op = ot_new_op();
    ...
    mu_assert(
        cmp == 0,
        "Serializing a single openElement did not create expected string.");
}
...
MU_TEST(test_serialize_single_close_element) {
    ...
    mu_assert(
        cmp == 0,
        "Serializing a single closeElement did not create expected string.");
}
...
MU_TEST(encode_empty_doc) {
    ...
    if (cmp != 0) {
        char* msg;
        asprintf(&msg, "Expected the encoded document to be \"%s\", but "
                       "instead got \"%s\".",
                 EXPECTED, actual);
        mu_fail(msg);
    }
...
MU_TEST(hextoa_does_not_write_to_array_when_hex_is_empty) {
    ...
    mu_assert(EXPECTED == ACTUAL, "Converting an empty hex string to an array "
                                  "should not write anything to the array.");
}
...
MU_TEST(hextoa_decodes_mixed_case_letters) {
    ...
    mu_assert(cmp == 0,
              "Decoding mixed-case letters gave an incorrect result.");
}
...
MU_TEST(atohex_encodes_single_byte) {
    ...
    char msg[1024];
    sprintf(msg, "Encoding a single byte gave an incorrect result. Expected = "
                 "\"%s\". Actual = \"%.2s\".",
            EXPECTED, actual);
    mu_assert(cmp == 0, msg);
}
...
MU_TEST(atohex_encodes_multiple_bytes) {
    ...
    char msg[1024];
    sprintf(msg, "Encoding multiple bytes gave an incorrect result. Expected = "
                 "\"%s\". Actual = \"%.6s\".",
            EXPECTED, actual);
    mu_assert(cmp == 0, msg);
}
...
sha1_test sha1_tests[] = {
    (sha1_test){"", (char[20]){0xda, 0x39, 0xa3, 0xee, 0x5e, 0x6b, 0x4b, 0x0d,
                               0x32, 0x55, 0xbf, 0xef, 0x95, 0x60, 0x18, 0x90,
                               0xaf, 0xd8, 0x07, 0x09}},
    (sha1_test){"hello world",
                (char[20]){0x2a, 0xae, 0x6c, 0x35, 0xc9, 0x4f, 0xcf, 0xb4, 0x15,
                           0xdb, 0xe9, 0x5f, 0x40, 0x8b, 0x9c, 0xe9, 0x1e, 0xe8,
                           0x46, 0xed}},
    (sha1_test){"lorem ipsum dolor sit amet consetetur sadipscing elitr sed "
                "diam nonumy eirmod tempor invidunt ut labore et dolore magna "
                "aliquyam",
                (char[20]){0x78, 0x34, 0x63, 0x8a, 0xa8, 0xeb, 0x18, 0xbb, 0x43,
                           0x95, 0x2d, 0x91, 0x9e, 0xfb, 0x49, 0x1e, 0xe4, 0x82,
                           0x56, 0x45}}};
...
MU_TEST(sha1) {
    ...
    for (size_t i = 0; i < max; ++i) {
        ...
        mu_assert(result == CRYPT_OK,
                  "An error occurred initializing the hash state.");

        result = sha1_process(&hs, t.in, (uint32_t)strlen(t.in));
        mu_assert(result == CRYPT_OK,
                  "An error occurred processing the hash input.");
...
MU_TEST(client_has_correct_state_and_event_when_receiving_op_for_empty_doc) {
    ...
    char* enc_op = "{ \"clientId\": 0, \"parent\": \"00\", \"hash\": \"00\", "
                   "\"components\": [ { \"type\": \"insert\", \"text\": \"any "
                   "string\" } ] }";
    ...
    if (cmp != 0) {
        ...
        asprintf(
            &msg,
            "Expected the document state to be \"%s\", but instead got \"%s\".",
            EXPECTED, actual);
        mu_fail(msg);
    }
    ...
    if (event_type != OT_OP_APPLIED) {
        ...
        asprintf(
            &msg,
            "Expected an event of type OT_OP_APPLIED (%d), but instead got %d.",
            OT_OP_APPLIED, event_type);
        mu_fail(msg);
    }
...
MU_TEST(
    client_has_correct_state_and_event_when_receiving_op_for_non_empty_doc) {
    const uint32_t NONZERO = 1;
    const char* const EXPECTED = "abcdef";
    const char* const ENC_OP1 = "{ \"clientId\": 0, \"parent\": \"00\", "
                                "\"hash\": \"00\", \"components\": [ { "
                                "\"type\": \"insert\", \"text\": \"abc\" } ] }";
    const char* const ENC_OP2 = "{ \"clientId\": 0, \"parent\": \"00\", "
                                "\"hash\": \"00\", \"components\": [ { "
                                "\"type\": \"skip\", \"count\": 3 }, { "
...
MU_TEST(client_receive_does_not_send_empty_buffer_after_acknowledgement) {
    ...
    if (sent_op != nothing) {
        ...
        asprintf(
            &msg,
            "Expected the client to not send anything, but it sent \"%s\".",
            sent_op);
        mu_fail(msg);
    }
...
MU_TEST(client_apply_sends_op_if_not_waiting_for_acknowledgement) {
    ...
    mu_assert(ot_equal(op, dec_sent_op),
              "Sent op wasn't equal to the applied op.");
...
MU_TEST(client_receives_new_op_before_acknowledgement_starting_with_empty_doc) {
    ...
    char* enc_serv_op = "{ \"clientId\": 1, \"parent\": \"00\", \"hash\": "
                        "\"2a19fa462b801abce17bf5bb6c13f370c268d9b2\", "
                        "\"components\": [ { \"type\": \"insert\", \"text\": "
                        "\"server text \" } ] }";
    ...
    if (cmp != 0) {
        char* msg;
        asprintf(&msg, "Expected the client's document to be \"%s\", but "
                       "instead it's \"%s\".",
                 EXPECTED, actual);
        mu_fail(msg);
    }
...
MU_TEST(
    client_receives_multiple_ops_before_acknowledgement_starting_with_empty_doc) {
    ...
    char* enc_serv_op = "{ \"clientId\": 1, \"parent\": \"00\", \"hash\": "
                        "\"d82ac619d64a0883de5276f0f3e9a984c3e22620\", "
                        "\"components\": [ { \"type\": \"insert\", \"text\": "
                        "\"server text\" } ] }";
    char* enc_serv_op2 = "{ \"clientId\": 1, \"parent\": \"00\", \"hash\": "
                         "\"66a02881b5b0d0d4e2e40b2bfa3d6e3ca710c85c\", "
                         "\"components\": [ { \"type\": \"skip\", \"count\": "
                         "11 }, { \"type\": \"insert\", \"text\": \" more "
                         "server text \" } ] }";
    ot_client_receive(client, enc_serv_op);
...
MU_TEST(
    client_receives_new_op_before_acknowledgement_and_then_applies_local_op) {
...
MU_TEST_SUITE(client_test_suite) {
    MU_RUN_TEST(
        client_has_correct_state_and_event_when_receiving_op_for_empty_doc);
    MU_RUN_TEST(
        client_has_correct_state_and_event_when_receiving_op_for_non_empty_doc);
    MU_RUN_TEST(
        client_receive_does_not_send_empty_buffer_after_acknowledgement);
    MU_RUN_TEST(client_apply_sends_op_if_not_waiting_for_acknowledgement);
    MU_RUN_TEST(
        client_receives_new_op_before_acknowledgement_starting_with_empty_doc);
    MU_RUN_TEST(
        client_receives_multiple_ops_before_acknowledgement_starting_with_empty_doc);
    MU_RUN_TEST(
        client_receives_new_op_before_acknowledgement_and_then_applies_local_op);
